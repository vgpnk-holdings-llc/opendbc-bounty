...
static void subaru_rx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == MSG_SUBARU_Steering_Torque) {  // only whitelisted on the main bus
  ...
  if (msg->addr == MSG_SUBARU_CruiseControl) {  // only whitelisted on alt_main_bus
    bool cruise_engaged = (msg->data[5] >> 1) & 1U;
    pcm_cruise_check(cruise_engaged);
  }

  // update vehicle moving with any non-zero wheel speed
  if (msg->addr == MSG_SUBARU_Wheel_Speeds) {  // only whitelisted on alt_main_bus
  ...
  if (msg->addr == MSG_SUBARU_Brake_Status) {  // only whitelisted on alt_main_bus
    brake_pressed = (msg->data[7] >> 6) & 1U;
  }

  if (msg->addr == MSG_SUBARU_Throttle) {  // only whitelisted on the main bus
