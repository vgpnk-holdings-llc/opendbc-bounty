...
static uint8_t volkswagen_pq_get_counter(const CANPacket_t *msg) {
  ...
  } else {  // MSG_GRA_NEU, the only other counter-checked message
...
static void volkswagen_pq_rx_hook(const CANPacket_t *msg) {
  ...
  if (volkswagen_longitudinal) {
    ...
    if (msg->addr == MSG_GRA_NEU) {
      ...
      bool set_falling = volkswagen_set_button_prev && !set_button;
      bool resume_falling = volkswagen_resume_button_prev && !resume_button;
      if (set_falling || resume_falling) {
