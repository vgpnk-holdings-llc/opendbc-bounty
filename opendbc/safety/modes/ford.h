...
static uint8_t ford_get_counter(const CANPacket_t *msg) {
  ...
  } else {  // FORD_Yaw_Data_FD1, the only other counter-checked message
...
static uint32_t ford_get_checksum(const CANPacket_t *msg) {
  ...
  if (msg->addr == FORD_BrakeSysFeatures) {
    ...
    chksum = msg->data[3];
  } else {  // FORD_Yaw_Data_FD1, the only other checksummed message
...
static bool ford_get_quality_flag_valid(const CANPacket_t *msg) {
  ...
  } else {  // FORD_Yaw_Data_FD1, the only other quality-flag-checked message
...
static void ford_rx_hook(const CANPacket_t *msg) {
  // every rx check is on the main bus
