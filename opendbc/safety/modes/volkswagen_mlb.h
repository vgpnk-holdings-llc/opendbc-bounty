...
static void volkswagen_mlb_rx_hook(const CANPacket_t *msg) {
  ...
  if (ADDR_BUS_MATCH(msg, MSG_ESP_03, 0U)) {
    uint32_t speed = 0;
  ...
  if (ADDR_BUS_MATCH(msg, MSG_LH_EPS_03, 0U)) {
    update_sample(&torque_driver, volkswagen_mlb_mqb_driver_input_torque(msg));
  }

  if (ADDR_BUS_MATCH(msg, MSG_LS_01, 0U)) {
  ...
  if (ADDR_BUS_MATCH(msg, MSG_MOTOR_03, 0U)) {
    gas_pressed = msg->data[6] != 0U;
    volkswagen_brake_pedal_switch = GET_BIT(msg, 35U);
  }

  if (ADDR_BUS_MATCH(msg, MSG_ESP_05, 0U)) {
  ...
  if (ADDR_BUS_MATCH(msg, MSG_TSK_04, 1U)) {
