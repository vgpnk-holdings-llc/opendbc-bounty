...
void hyundai_common_cruise_state_check(const bool cruise_engaged) {
  ...
  // only called when openpilot longitudinal is disabled: the CAN FD call site checks
  // !hyundai_longitudinal and the legacy call site's rx checks only whitelist SCC12
  // for non-longitudinal configs
...
uint32_t hyundai_common_canfd_compute_checksum(const CANPacket_t *msg) {
  ...
  // only 24- and 32-byte messages are whitelisted in the rx checks, so the
  // non-24 XOR seed is always the 32-byte one in contract; other lengths are out of contract
  crc ^= (len == 24) ? 0x819dU : 0x9f5bU;
