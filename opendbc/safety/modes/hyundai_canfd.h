...
static void hyundai_canfd_rx_hook(const CANPacket_t *msg) {
  ...
  if (msg->bus == pt_bus) {
    ...
    if ((msg->addr == 0x35U) && hyundai_ev_gas_signal) {
      ...
      gas_pressed = GET_BIT(msg, 103U);
      gas_pressed |= (msg->data[13] != 0U);
      gas_pressed |= GET_BIT(msg, 112U);
  ...
  if (msg->bus == scc_bus) {
    // cruise state (SCC_CONTROL is only whitelisted for stock cars)
    if (msg->addr == 0x1a0U) {
...
static bool hyundai_canfd_tx_hook(const CANPacket_t *msg) {
  ...
  bool uds_lka_steer = (msg->addr == 0x730U);  // 0x730 is only whitelisted for LKA-steering cars
  bool uds_radar = (msg->addr == 0x7D0U);  // 0x7D0 is only whitelisted for radar-SCC cars
  if (uds_lka_steer || uds_radar) {
