...
static uint8_t hyundai_get_counter(const CANPacket_t *msg) {
  ...
  if (msg->addr == 0x260U) {
    ...
    cnt = msg->data[7] & 0xFU;
  } else {  // 0x4F1, the only other counter-checked message
...
static uint32_t hyundai_get_checksum(const CANPacket_t *msg) {
  ...
  } else {  // 0x421, the only other checksummed message
    chksum = msg->data[7] >> 4;
  }
  return chksum;
}
...
static void hyundai_rx_hook(const CANPacket_t *msg) {
  ...
  // SCC12 is only whitelisted on the SCC bus (bus 2 for camera-based SCC cars, bus 0 otherwise)
  if (msg->addr == 0x421U) {
    // 2 bits: 13-14
  ...
  if (msg->bus == 0U) {
    ...
    } else if (msg->addr == 0x91U) {  // 0x91 is only whitelisted for FCEV cars
