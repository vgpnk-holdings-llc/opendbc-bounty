...
static void honda_rx_hook(const CANPacket_t *msg) {
  ...
  // 0x1A6 for the ILX, 0x296 for the Civic Touring (only whitelisted on the PT bus)
  ...
  if (!(alternative_experience & ALT_EXP_DISABLE_STOCK_AEB)) {
    // 0x1FA (BRAKE_COMMAND) is the only message whitelisted on bus 2
    if (msg->bus == 2U) {
...
static bool honda_tx_hook(const CANPacket_t *msg) {
  ...
  // note: every tx msg check below matches a message whitelisted on a single bus

  // ACC_HUD: safety check (nidec w/o pedal)
  if (msg->addr == 0x30CU) {  // only whitelisted on the PT bus
  ...
  if (msg->addr == 0x1FAU) {  // only whitelisted on the PT bus
  ...
  if (msg->addr == 0x1DFU) {  // only whitelisted on the PT bus
  ...
  if (msg->addr == 0x1C8U) {  // only whitelisted on the PT bus
  ...
  if ((msg->addr == 0x296U) && !controls_allowed) {  // only whitelisted on bus_buttons
