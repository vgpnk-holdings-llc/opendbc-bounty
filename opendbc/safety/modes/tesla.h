...
static uint8_t tesla_get_counter(const CANPacket_t *msg) {
  ...
  if (msg->addr == 0x2b9U) {
    ...
    cnt = msg->data[6] >> 4;
  } else {  // 0x370, the only other counter-checked message
...
static int _tesla_get_checksum_byte(const int addr) {
  ...
  if ((addr == 0x370) || (addr == 0x2b9) || (addr == 0x155)) {
    ...
    checksum_byte = 3;
  } else if ((addr == 0x257) || (addr == 0x118) || (addr == 0x145) || (addr == 0x286)) {
    // Signal: DI_speedChecksum, DI_systemStatusChecksum, ESP_statusChecksum, DI_locStatusChecksum
    checksum_byte = 0;
  } else {  // 0x311, the only other checksummed message
    // Signal: UI_warningChecksum
    checksum_byte = 0;
  }
...
static uint32_t tesla_get_checksum(const CANPacket_t *msg) {
  uint8_t chksum = 0;
  // every checksummed message is in the table
...
static bool tesla_get_quality_flag_valid(const CANPacket_t *msg) {
  ...
  } else {  // 0x145, the only other quality-flag-checked message
...
static int tesla_get_steer_ctrl_type(const int ctrl_type) {
  ...
  // swap control types for FSD 14 (only ever called with 1 or 2)
  int steer_ctrl_type = ctrl_type;
  if (tesla_fsd_14) {
    steer_ctrl_type = 3 - ctrl_type;
  }
...
static void tesla_rx_hook(const CANPacket_t *msg) {
  ...
  if (msg->bus == 0U) {
    ...
    if (msg->addr == 0x286U) {
      ...
      cruise_engaged |= (cruise_state == 3);        // STANDSTILL
      cruise_engaged |= (cruise_state == 4);        // OVERRIDE
      cruise_engaged |= (cruise_state == 6);        // PRE_FAULT
      cruise_engaged |= (cruise_state == 7);        // PRE_CANCEL
