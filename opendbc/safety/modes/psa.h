...
static uint32_t psa_compute_checksum(const CANPacket_t *msg) {
  // assumes exactly two checksummed whitelisted addresses; the else case is PSA_HS2_DYN_ABR_38D
...
static bool psa_tx_hook(const CANPacket_t *msg) {
  ...
  // Safety check for LKA; only PSA_LANE_KEEP_ASSIST is whitelisted in the tx msgs
  // SET_ANGLE
