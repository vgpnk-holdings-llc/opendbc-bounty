...
static uint8_t psa_get_counter(const CANPacket_t *msg) {
  ...
  } else {  // PSA_HS2_DYN_ABR_38D, the only other counter-checked message
...
static uint32_t psa_compute_checksum(const CANPacket_t *msg) {
  ...
  } else {  // PSA_HS2_DYN_ABR_38D, the only other checksummed message
...
static void psa_rx_hook(const CANPacket_t *msg) {
  ...
  // PSA_HS2_DAT_MDD_CMD_452 (cruise state) is the only rx check on the ADAS bus
  if (msg->bus == PSA_ADAS_BUS) {
    pcm_cruise_check((msg->data[2U] >> 7U) & 1U); // RVV_ACC_ACTIVATION_REQ
  }


  // PSA_DAT_BSI (brake) is the only rx check on the camera bus
