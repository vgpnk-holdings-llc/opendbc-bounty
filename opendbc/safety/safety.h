...
// index must come from get_addr_check_index and be != -1 (only caller checks)
...
static bool rx_msg_safety_check(const CANPacket_t *msg,
  ...
  if (index != -1) {
    // checksum check (hook tables always define get_checksum and compute_checksum as a pair)
    ...
    if ((safety_hooks->get_counter != NULL) && (cfg->rx_checks[index].msg[cfg->rx_checks[index].status.index].max_counter > 0U)) {
      ...
      // every rx check without a counter sets ignore_counter, so this branch is unconditional
...
void safety_tick(const safety_config *cfg) {
  ...
  if (cfg != NULL) {
    for (int i=0; i < cfg->rx_checks_len; i++) {
      ...
      // lag threshold: 1s. Every rx check is >=10Hz, so this is always >=10
      // missed messages. 2s of lag is worst case at the 1Hz tick rate
      bool lagging = elapsed_time > 1e6;
      cfg->rx_checks[i].status.lagging = lagging;
      bool msg_invalid = !is_msg_valid(cfg->rx_checks, i);
      if (lagging) {
...
int set_safety_hooks(uint16_t mode, uint16_t param) {
  ...
  // every entry in safety_hook_registry defines init
  if (set_status == 0) {
