...
class AngleSteeringSafetyTest(VehicleSpeedSafetyTest):
  ...
  def test_angle_cmd_meas_outside_limits(self):
    # measured angles outside the steer limits are clamped in the inactive-angle check
    for controls_allowed in (True, False):
      self.safety.set_controls_allowed(controls_allowed)
      for angle_meas in (-self.STEER_ANGLE_MAX * 2, self.STEER_ANGLE_MAX * 2):
        self._reset_angle_measurement(angle_meas)
        clamped = sign_of(angle_meas) * self.STEER_ANGLE_MAX
        self._set_prev_desired_angle(clamped)
        # desired angle within the clamped boundary is allowed, beyond it is not
        self.assertTrue(self._tx(self._angle_cmd_msg(clamped, False)))
        self.assertFalse(self._tx(self._angle_cmd_msg(clamped + sign_of(angle_meas) * 5, False)))
