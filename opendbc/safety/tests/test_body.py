...
class TestBody(common.SafetyTest):
  ...
  def test_can_flasher(self):
    ...
    self.assertFalse(self._tx(common.make_msg(0, 0x250, dat=b'\xce\xfa\xad\xde\x00\x00\x00\x00')))  # only first magic
    msg = common.make_msg(0, 0x250, dat=b'\xce\xfa\xad\xde\x1e\x0b\xb0\x0a')
    msg[0].data_len_code = 6  # DLC < 8: magic in the buffer must be ignored
