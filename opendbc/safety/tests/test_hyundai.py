...
class TestHyundaiSafety(HyundaiButtonBase, common.CarSafetyTest, common.DriverTorqueSteeringSafetyTest, common.SteerRequestCutSafetyTest):
  ...
  def test_foreign_gas_msgs_ignored(self):
    # other powertrain variants' gas messages are whitelisted but must not set gas
    self._rx(self.packer.make_can_msg_safety("E_EMS11", 0, {}))
    self.assertFalse(self.safety.get_gas_pressed_prev())
    self._rx(self.packer.make_can_msg_safety("FCEV_ACCELERATOR", 0, {}))
    self.assertFalse(self.safety.get_gas_pressed_prev())

  def test_vehicle_moving_rear_right(self):
    # rear-right wheel alone marks the vehicle moving
    values = {"WHL_SPD_FL": 0, "WHL_SPD_FR": 0, "WHL_SPD_RL": 0, "WHL_SPD_RR": 100,
  ...
  def test_cruise_engagement_with_recent_button(self):
    if isinstance(self, HyundaiLongitudinalBase):
      self.skipTest("longitudinal cars use button engagement")
    # a recent button press plus a rising cruise edge engages
    self._rx(self._button_msg(1))
    self._rx(self._pcm_status_msg(False))
    self._rx(self._pcm_status_msg(True))
    self.assertTrue(self.safety.get_controls_allowed())
    # holding cruise on must not re-trigger the rising edge
    self._rx(self._pcm_status_msg(True))
    self.assertTrue(self.safety.get_controls_allowed())

  def test_cruise_engagement_requires_recent_button(self):
    if isinstance(self, HyundaiLongitudinalBase):
      self.skipTest("longitudinal cars use button engagement")
    # with a stale button interaction, a rising cruise edge must not engage
    self._rx(self._pcm_status_msg(True))
...
class TestHyundaiLongitudinalSafety(HyundaiLongitudinalBase, TestHyundaiSafety):
  ...
  def test_foreign_msgs_ignored_when_long(self):
    # buttons and SCC12 are whitelisted but take the longitudinal path
    self._rx(self._button_msg(4))
    self.assertFalse(self.safety.get_controls_allowed())
    # SCC12 is not whitelisted for longitudinal cars, so it takes no code path
    self._rx(self.packer.make_can_msg_safety("SCC12", self.SCC_BUS, {"ACC_Enable": 1}, fix_checksum=checksum))
    self.assertFalse(self.safety.get_controls_allowed())

  def test_buttons_tx_allowed_when_long(self):
    # longitudinal cars take the non-gated path for CLU11 tx
    self.assertTrue(self._tx(self._button_msg(1)))

  def test_tester_present_trailing_bytes(self):
    # only exact tester present is allowed on the UDS address
    self.assertFalse(self._tx(common.make_msg(0, 0x7D0, 8, b"\x02\x3E\x80\x00\x00\x00\x00\x01")))
