...
class TestHondaNidecSafetyBase(HondaBase):
  ...
  def test_brake_switch_two_frames(self):
    # brake switch must be set for two consecutive frames to count as pressed
    for _ in range(2):
      self.assertFalse(self.safety.get_brake_pressed_prev())
      self._rx(self._powertrain_data_msg(brake_switch=1))
    self.assertTrue(self.safety.get_brake_pressed_prev())

  def test_stock_aeb_disabled_in_alt_exp(self):
    # stock AEB forwarding must be off when ALT_EXP_DISABLE_STOCK_AEB is set
    self.safety.set_alternative_experience(2)  # ALT_EXP_DISABLE_STOCK_AEB
    self._rx(self._rx_brake_msg(self.MAX_BRAKE, aeb_req=1))
    self.assertFalse(self.safety.get_honda_fwd_brake())

  def test_steer_motor_torque_safety_check(self):
    # 0x194 follows the same applied-while-disabled rule as 0xE4
    self.safety.set_controls_allowed(False)
    self.assertFalse(self._tx(common.make_msg(0, 0x194, 4, b"\x00\x01\x00\x00")))
    self.assertTrue(self._tx(common.make_msg(0, 0x194, 4, b"\x00\x00\x00\x00")))
    self.safety.set_controls_allowed(True)
    self.assertTrue(self._tx(common.make_msg(0, 0x194, 4, b"\x00\x01\x00\x00")))
...
class TestHondaBoschSafety(HondaPcmEnableBase, TestHondaBoschSafetyBase):
  ...
  def test_brake_hold_payload(self):
    # BRAKE_HOLD must carry exactly the supplemental-control magic
    self.assertTrue(self._tx(common.make_msg(0, 0xE5, 8, b"\x04\x00\x80\x10\x00\x00\x00\x00")))
    self.assertFalse(self._tx(common.make_msg(0, 0xE5, 8, b"\x05\x00\x80\x10\x00\x00\x00\x00")))
    self.assertFalse(self._tx(common.make_msg(0, 0xE5, 8, b"\x04\x00\x80\x10\x01\x00\x00\x00")))
...
class TestHondaBoschLongSafety(HondaButtonEnableBase, TestHondaBoschSafetyBase):
  ...
  def test_diagnostics(self):
    ...
    # trailing bytes must also be zero
    trailing_bytes = libsafety_py.make_CANPacket(0x18DAB0F1, self.PT_BUS, b"\x02\x3E\x80\x00\x00\x00\x00\x01")
