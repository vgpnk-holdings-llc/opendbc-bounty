...
class TestTeslaSafetyBase(common.CarSafetyTest, common.AngleSteeringSafetyTest, common.LongitudinalAccelSafetyTest):
  ...
  def test_brake_quality_flag_sna(self):
    # ESP_driverBrakeApply = 3 (SNA) must fail the quality check
    self._rx(self._user_brake_msg(0))
    self.assertTrue(self._rx(self.packer.make_can_msg_safety("ESP_status", 0, {"ESP_driverBrakeApply": 1})))
    self.assertFalse(self._rx(self.packer.make_can_msg_safety("ESP_status", 0, {"ESP_driverBrakeApply": 3})))

  def test_cruise_states_engaged(self):
    # cruise states 3 (standstill), 4 (override) and 6 (pre-fault) also mean engaged
    for state in (3, 4, 6):
      self._rx(self._pcm_status_msg(False))
      self._rx(self.packer.make_can_msg_safety("DI_state", 0, {"DI_cruiseState": state}))
      self.assertTrue(self.safety.get_controls_allowed(), state)

  def test_ui_warning_counter(self):
    # UI_warning is counter-checked: consecutive counters must be accepted
    for i in range(3):
      self.assertTrue(self._rx(self.packer.make_can_msg_safety("UI_warning", 0, {"UI_warningCounter": i})))

  def test_stock_lkas_edge_cases(self):
    if self.SAFETY_PARAM & TeslaSafetyFlags.FSD_14:
      self.skipTest("fwd hook differs on FSD 14")
    # stock LKAS latches only on a rising edge while controls are not allowed,
    # observable through the fwd hook (0x488 unblocked while latched)
    lkas = {"DAS_steeringControlType": self.steer_control_types["LANE_KEEP_ASSIST"]}
    self.assertTrue(self.safety.safety_fwd_hook(2, 0x488))
    self._rx(self.packer.make_can_msg_safety("DAS_steeringControl", 2, lkas))
    self.assertFalse(self.safety.safety_fwd_hook(2, 0x488))
    # held level: no new rising edge, still latched
    self._rx(self.packer.make_can_msg_safety("DAS_steeringControl", 2, lkas))
    self.assertFalse(self.safety.safety_fwd_hook(2, 0x488))

    # controls allowed: rising edge ignored
    self.safety.init_tests()
    self.safety.set_controls_allowed(True)
    self._rx(self.packer.make_can_msg_safety("DAS_steeringControl", 2, {"DAS_steeringControlType": 0}))
    self._rx(self.packer.make_can_msg_safety("DAS_steeringControl", 2, lkas))
    self.assertTrue(self.safety.safety_fwd_hook(2, 0x488))
    ...
    for hands_on_level in range(4):
      for eac_status in range(8):
        for eac_error_code in range(16):
          ...
          # stays disengaged while the condition persists (no re-trigger on the level)
          self.assertTrue(self._rx(self._angle_meas_msg(0, hands_on_level=hands_on_level, eac_status=eac_status,
                                                        eac_error_code=eac_error_code)))
          self.assertNotEqual(should_disengage, self.safety.get_controls_allowed())
  ...
  def test_fwd_hook_autopark(self):
    # with autopark active, eac monitor and stock steering are not blocked
    self._rx(self._pcm_status_msg(False, self.autopark_states["SELFPARK_STARTED"]))
    self.assertFalse(self.safety.safety_fwd_hook(2, 0x27D))
    self.assertFalse(self.safety.safety_fwd_hook(2, 0x488))
...
class TestTeslaFSD14StockSafety(TestTeslaStockSafety):
  ...
  def test_stock_lkas_ctrl_type_swap(self):
    # raw control types are remapped for FSD 14 vehicles
    self._rx(self.packer.make_can_msg_safety("DAS_steeringControl", 2, {"DAS_steeringControlType": 1}))
    self._rx(self.packer.make_can_msg_safety("DAS_steeringControl", 2, {"DAS_steeringControlType": 2}))
    self._rx(self.packer.make_can_msg_safety("DAS_steeringControl", 2, {"DAS_steeringControlType": 3}))
...
class TestTeslaIgnition(unittest.TestCase):
  ...
  def test_ignition_wrong_bus(self):
    self.safety.ignition_can_hook(self.packer.make_can_msg_safety("VCFRONT_LVPowerState", 2, {"VCFRONT_LVPowerStateCounter": 0, "VCFRONT_vehiclePowerState": 3}))
    self.safety.ignition_can_hook(self.packer.make_can_msg_safety("VCFRONT_LVPowerState", 2, {"VCFRONT_LVPowerStateCounter": 1, "VCFRONT_vehiclePowerState": 3}))
    self.assertFalse(self.safety.get_ignition_can())

  def test_ignition_wrong_len(self):
    # consecutive counters, drive state set, but DLC != 8: must be ignored
    self.safety.ignition_can_hook(common.make_msg(0, 0x221, 4, bytes([0x60, 0, 0, 0])))
    self.safety.ignition_can_hook(common.make_msg(0, 0x221, 4, bytes([0x60, 0, 0, 0])))
    self.assertFalse(self.safety.get_ignition_can())
