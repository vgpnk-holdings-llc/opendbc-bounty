...
class TestFordSafetyBase(common.CarSafetyTest):
  ...
  def test_cruise_state_4_engaged(self):
    # cruise state 4 (engaged) must enable controls just like state 5
...
class TestFordLongitudinalSafetyBase(TestFordSafetyBase):
  ...
  def test_brake_actuation_single_signal(self):
    # either brake actuation signal alone must block tx when controls are off
    self.safety.set_controls_allowed(False)
    for sig in ("AccBrkPrchg_B_Rq", "AccBrkDecel_B_Rq"):
      values = {"AccPrpl_A_Rq": self.INACTIVE_GAS, "AccPrpl_A_Pred": self.INACTIVE_GAS,
                "AccBrkTot_A_Rq": self.INACTIVE_ACCEL, "AccBrkPrchg_B_Rq": 0,
                "AccBrkDecel_B_Rq": 0, "CmbbDeny_B_Actl": 0, sig: 1}
