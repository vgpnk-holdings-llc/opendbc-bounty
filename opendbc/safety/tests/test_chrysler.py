...
class TestChryslerSafety(common.CarSafetyTest, common.MotorTorqueSteeringSafetyTest):
  ...
  def test_vehicle_moving_single_side(self):
    if type(self) is not TestChryslerSafety:
      self.skipTest("Pacifica speed message only")
    # either wheel speed alone marks the vehicle moving
    for values in ({"SPEED_LEFT": 0, "SPEED_RIGHT": 1}, {"SPEED_LEFT": 1, "SPEED_RIGHT": 0}):
