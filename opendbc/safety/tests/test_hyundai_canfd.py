...
class TestHyundaiCanfdBase(HyundaiButtonBase, common.CarSafetyTest, common.DriverTorqueSteeringSafetyTest, common.SteerRequestCutSafetyTest):
  ...
  def test_foreign_gas_msgs_ignored(self):
    # only this variant's gas message may set gas_pressed
    for name in ("ACCELERATOR", "ACCELERATOR_ALT", "ACCELERATOR_BRAKE_ALT"):
      self.setUp()  # reset wrong-counter state so each message is accepted
      self._rx(self.packer.make_can_msg_safety(name, self.PT_BUS, {}))
      own = name == self.GAS_MSG[0]
      sigs = {"ACCELERATOR": ("ACCELERATOR_PEDAL",), "ACCELERATOR_ALT": ("ACCELERATOR_PEDAL", "ACCELERATOR_PEDAL_PRESSED"),
              "ACCELERATOR_BRAKE_ALT": ("ACCELERATOR_PEDAL_PRESSED",)}[name]
      for sig in sigs:
        self._rx(self.packer.make_can_msg_safety(name, self.PT_BUS, {sig: 1}))
        self.assertEqual(own and sig == self.GAS_MSG[1], self.safety.get_gas_pressed_prev(), (name, sig))

  def test_vehicle_moving_single_wheel(self):
    # any single wheel above the threshold marks the vehicle moving
    for pos in ("FL", "FR", "RL", "RR"):
      ...
      self.assertTrue(self.safety.get_vehicle_moving(), pos)
      self._rx(self._speed_msg(0))
      self.assertFalse(self.safety.get_vehicle_moving())

  def test_cruise_status_2_engaged(self):
    # ACC mode 2 (driver override) also means engaged
    if isinstance(self, HyundaiLongitudinalBase):
      self.skipTest("longitudinal cars use button engagement")
    # engagement requires a recent button interaction
    self._rx(self._button_msg(1))
    self._rx(self.packer.make_can_msg_safety("SCC_CONTROL", self.SCC_BUS, {"ACCMode": 2}))
    self.assertTrue(self.safety.get_controls_allowed())

  def test_accel_values_blocked_when_stock(self):
    # stock cars may only send cancel (mode 4) with zero accel values
    if isinstance(self, HyundaiLongitudinalBase):
      self.skipTest("longitudinal sends real accel")
    self.assertFalse(self._tx(self.packer.make_can_msg_safety("SCC_CONTROL", 1, {"ACCMode": 4, "aReqRaw": 0, "aReqValue": 1})))
    self.assertFalse(self._tx(self.packer.make_can_msg_safety("SCC_CONTROL", 1, {"ACCMode": 3, "aReqRaw": 0, "aReqValue": 0})))
...
class TestHyundaiCanfdLFASteeringLongBase(HyundaiLongitudinalBase, TestHyundaiCanfdLFASteeringBase):
  ...
  TX_MSGS = TestHyundaiCanfdLFASteeringBase.TX_MSGS + [[0x7D0, 0], [0x160, 0]]
