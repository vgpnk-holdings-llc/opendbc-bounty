...
class TestVolkswagenMebStockSafety(TestVolkswagenMebSafetyBase):
  ...
  def test_vehicle_moving_single_wheel(self):
    # any single wheel above zero marks the vehicle moving
    for wheel in ("VL", "VR", "HL", "HR"):
      values = {f"{s}_Radgeschw": 0 for s in ("VL", "VR", "HL", "HR")}
      values[f"{wheel}_Radgeschw"] = 1
      self._rx(self.packer.make_can_msg_safety("ESC_51", 0, values))
      self.assertTrue(self.safety.get_vehicle_moving())
      self._rx(self._speed_msg(0))
      self.assertFalse(self.safety.get_vehicle_moving())

  def test_cruise_status_4_5_engaged(self):
    # ACC statuses 4 and 5 also mean engaged
    for status in (4, 5):
      self._rx(self.packer.make_can_msg_safety("Motor_51", 0, {"TSK_Status": status}))
      self.assertTrue(self.safety.get_controls_allowed())
...
class TestVolkswagenMebIgnition(unittest.TestCase):
  ...
  def test_ignition_wrong_bus(self):
    self.safety.ignition_can_hook(self.packer.make_can_msg_safety("Klemmen_Status_01", 2, {"Klemmen_Status_01_BZ": 0, "ZAS_Kl_15": 1}))
    self.safety.ignition_can_hook(self.packer.make_can_msg_safety("Klemmen_Status_01", 2, {"Klemmen_Status_01_BZ": 1, "ZAS_Kl_15": 1}))
    self.assertFalse(self.safety.get_ignition_can())

  def test_ignition_wrong_len(self):
    # consecutive counters, kl.15 set, but DLC != 4: must be ignored
    self.safety.ignition_can_hook(common.make_msg(0, 0x3C0, 8, bytes([0, 0, 2, 0, 0, 0, 0, 0])))
    self.safety.ignition_can_hook(common.make_msg(0, 0x3C0, 8, bytes([0, 1, 2, 0, 0, 0, 0, 0])))
    self.assertFalse(self.safety.get_ignition_can())
