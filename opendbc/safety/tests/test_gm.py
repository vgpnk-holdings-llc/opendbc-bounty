...
class GmLongitudinalBase(common.CarSafetyTest, common.LongitudinalGasBrakeSafetyTest):
  ...
  def test_gas_apply_when_disabled(self):
    # apply bit set while controls are off must be blocked
    self.safety.set_controls_allowed(False)
    self.assertFalse(self._tx(common.make_msg(0, 0x2CB, 8, bytes([1, 2, 0xC0, 0x30]) + b"\x00" * 4)))
...
class TestGmSafetyBase(common.CarSafetyTest, common.DriverTorqueSteeringSafetyTest):
  ...
  def test_vehicle_moving_single_wheel(self):
    # either rear wheel alone marks the vehicle moving
    for values in ({"RLWheelSpd": 0, "RRWheelSpd": 1}, {"RLWheelSpd": 1, "RRWheelSpd": 0}):
...
class TestGmCameraSafety(TestGmCameraSafetyBase):
  ...
  def test_buttons(self):
    ...
    # buttons message on the powertrain bus must be ignored on rx for pcm_cruise modes
    self._rx(self._pcm_status_msg(True))
    self._rx(self.packer.make_can_msg_safety("ASCMSteeringButton", 0, {"ACCButtons": Buttons.CANCEL}))
...
class TestGmIgnition(unittest.TestCase):
  ...
  def test_ignition_wrong_bus(self):
    self.safety.ignition_can_hook(self.packer.make_can_msg_safety("BCMGeneralPlatformStatus", 2, {"SystemPowerMode": 2}))
    self.assertFalse(self.safety.get_ignition_can())

  def test_ignition_wrong_len(self):
    self.safety.ignition_can_hook(common.make_msg(0, 0x1F1, 4, bytes([2]) + b"\x00" * 3))
    self.assertFalse(self.safety.get_ignition_can())
