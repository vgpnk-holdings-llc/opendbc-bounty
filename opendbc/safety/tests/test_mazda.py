...
class TestMazdaSafety(common.CarSafetyTest, common.DriverTorqueSteeringSafetyTest):
  ...
  def test_gas_pedal_sources(self):
    # gas is detected from either ENGINE_DATA byte 4 or the high nibble of byte 5
    for dat in (bytes([0, 0, 0, 0, 1]) + b"\x00" * 3, bytes([0, 0, 0, 0, 0, 0x10]) + b"\x00" * 2):
      self._rx(make_msg(0, 0x202, dat=dat))
      self.assertTrue(self.safety.get_gas_pressed_prev())
      self._rx(make_msg(0, 0x202, dat=b"\x00" * 8))
...
class TestMazdaIgnition(unittest.TestCase):
  ...
  def test_ignition_wrong_bus(self):
    self.safety.ignition_can_hook(make_msg(2, 0x9E, dat=bytes([0xC0]) + b"\x00" * 7))
    self.assertFalse(self.safety.get_ignition_can())

  def test_ignition_wrong_len(self):
    self.safety.ignition_can_hook(make_msg(0, 0x9E, 4, bytes([0xC0]) + b"\x00" * 3))
    self.assertFalse(self.safety.get_ignition_can())
