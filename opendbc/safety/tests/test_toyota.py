...
class TestToyotaSafetyBase(common.CarSafetyTest, common.LongitudinalAccelSafetyTest):
  ...
  def test_angle_meas_ignored_while_initializing(self):
    # non-LTA cars ignore the angle measurement while the sensor is initializing
    self._rx(self.packer.make_can_msg_safety("STEER_TORQUE_SENSOR", 0, {"STEER_ANGLE_INITIALIZING": 1, "STEER_ANGLE": 100}))
  ...
  def test_diagnostics(self, stock_longitudinal: bool = False, ecu_disabled: bool = True):
    for should_tx, msg in ((False, b"\x6D\x02\x3E\x00\x00\x00\x00\x00"),  # fwdCamera tester present
                           ...
                           (False, b"\x0F\x02\x3E\x00\x00\x00\x00\x01")):  # trailing bytes must be zero
  ...
  def test_block_aeb_payload_bytes(self):
    # bytes 4 and 5 must also be zero to pass the AEB payload check
    for dat in (bytes([0, 0, 0, 0, 1, 0, 0]), bytes([0, 0, 0, 0, 0, 1, 0])):
      self.assertFalse(self._tx(libsafety_py.make_CANPacket(0x283, 0, dat)))
