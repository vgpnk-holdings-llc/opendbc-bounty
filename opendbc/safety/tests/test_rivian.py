...
class TestRivianIgnition(unittest.TestCase):
  ...
  def test_ignition_wrong_bus(self):
    self.safety.ignition_can_hook(self.packer.make_can_msg_safety("VDM_OutputSignals", 2, {"VDM_OutputSigs_Counter": 0, "VDM_EpasPowerMode": 1}))
    self.safety.ignition_can_hook(self.packer.make_can_msg_safety("VDM_OutputSignals", 2, {"VDM_OutputSigs_Counter": 1, "VDM_EpasPowerMode": 1}))
    self.assertFalse(self.safety.get_ignition_can())

  def test_ignition_wrong_len(self):
    # consecutive counters, drive mode set, but DLC != 8: must be ignored
    self.safety.ignition_can_hook(common.make_msg(0, 0x152, 4, bytes([0, 0, 0, 0x10])))
    self.safety.ignition_can_hook(common.make_msg(0, 0x152, 4, bytes([0, 1, 0, 0x10])))
    self.assertFalse(self.safety.get_ignition_can())
