...
class TestVolkswagenMlbSafetyBase(common.CarSafetyTest, common.DriverTorqueSteeringSafetyTest):
  ...
  def test_cruise_status_2_engaged(self):
    # ACC status 2 also means engaged
    self._rx(self.packer.make_can_msg_safety("TSK_04", 1, {"TSK_Status_GRA_ACC_02": 2}))
  ...
  def test_cancel_button_exits_controls(self):
    # rising edge of the LS_01 cancel button must exit controls
    self.safety.set_controls_allowed(True)
    self._rx(self.packer.make_can_msg_safety("LS_01", 0, {}))
    self.assertTrue(self.safety.get_controls_allowed())
    self._rx(self.packer.make_can_msg_safety("LS_01", 0, {"LS_Abbrechen": 1}))
    self.assertFalse(self.safety.get_controls_allowed())

  def test_steer_req_status_5(self):
    # steer request status 5 is treated like status 7: nonzero torque blocked when off
    self.safety.set_controls_allowed(False)
    msg = self.packer.make_can_msg_safety("HCA_01", 0, {"HCA_01_LM_Offset": 10, "HCA_01_Sendestatus": 1, "HCA_01_Status_HCA": 5})
